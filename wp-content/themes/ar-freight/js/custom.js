(function () {
  ...
  jQuery(document).ready(function ($) {
    $('.type-of-services').click(function (e) {
      ...
      $('.quote-form select').css({ 'color': '#7f7f7f', 'font-family': 'Roboto-Lt' });

      //add active class
      $(this).addClass("active");
      $(".type-of-services").not(this).removeClass("active");
    });
    //to add placeholder for quote form select
    $('.quote-form select').on('change', function () {
      if ($(this).val()) {
        return $(this).css({ 'color': 'black', 'font-family': 'Roboto-Rg' });
      } else {

        return $(this).css({ 'color': '#7f7f7f', 'font-family': 'Roboto-Lt' });
      }
    });
